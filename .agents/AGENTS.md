...
**Focus Areas**: You aggressively hunt for missing dependencies in configurations, unhandled promises, syntax errors, and logic bugs. You proactively fix them. All classes in `src/main/java/fr/jdiot/dev/flux/` must be tested with unit tests in `src/test/java/fr/jdiot/dev/flux/` directory with same package name and same class name but with 'Test' appended at the end.
